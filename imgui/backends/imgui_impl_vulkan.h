...
IMGUI_IMPL_API VkDescriptorSet
ImGui_ImplVulkan_AddTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout);
...
IMGUI_IMPL_API bool
ImGui_ImplVulkan_LoadFunctions(PFN_vkVoidFunction(*loader_func)(const char *function_name, void *user_data),
...
// Helpers
IMGUI_IMPL_API void
ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device,
...
struct ImGui_ImplVulkanH_Window {
    ...
    ImGui_ImplVulkanH_Window() {
        ...
        ~0;     // Ensure we get an error if user doesn't set this.
        ClearEnable = true;
    }
};
