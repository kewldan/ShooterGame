...
namespace {
    ...
    struct FreeTypeFont {
        ...
        void SetPixelHeight(
                int pixel_height); // Change font pixel size. All following calls to RasterizeGlyph() will use this size
        const FT_Glyph_Metrics *LoadGlyph(uint32_t in_codepoint);

        const FT_Bitmap *RenderGlyphAndGetInfo(GlyphInfo *out_glyph_info);

        void
        BlitGlyph(const FT_Bitmap *ft_bitmap, uint32_t *dst, uint32_t dst_pitch, unsigned char *multiply_table = NULL);
...
#define FT_CEIL(X)  (((X + 63) & -64) / 64)
    ...
    void FreeTypeFont::SetPixelHeight(int pixel_height) {
        ...
        req.type = (UserFlags & ImGuiFreeTypeBuilderFlags_Bitmap) ? FT_SIZE_REQUEST_TYPE_NOMINAL
    ...
    void FreeTypeFont::BlitGlyph(const FT_Bitmap *ft_bitmap, uint32_t *dst, uint32_t dst_pitch,
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas *atlas, unsigned int extra_flags) {
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++) {
            ...
            IM_ASSERT(buf_bitmap_current_used_bytes + bitmap_size_in_bytes <=
                      BITMAP_BUFFERS_CHUNK_SIZE); // We could probably allocate custom-sized buffer instead.
