...
static void sSendMouseCallback(uSynergyContext *context) {
    ...
    context->m_mouseCallback(context->m_cookie, context->m_mouseX, context->m_mouseY, context->m_mouseWheelX,
                             context->m_mouseWheelY, context->m_mouseButtonLeft, context->m_mouseButtonRight,
...
**/
static void sSendKeyboardCallback(uSynergyContext *context, uint16_t key, uint16_t modifiers, uSynergyBool down,
                                  uSynergyBool repeat) {
    // Skip if no callback is installed
...
static void sSendJoystickCallback(uSynergyContext *context, uint8_t joyNum) {
    ...
    context->m_joystickCallback(context->m_cookie, joyNum, context->m_joystickButtons[joyNum], sticks[0], sticks[1],
                                sticks[2], sticks[3]);
}
...
static void sUpdateContext(uSynergyContext *context) {
    ...
    int num_received = 0;
    int packlen = 0;
    if (context->m_receiveFunc(context->m_cookie, context->m_receiveBuffer + context->m_receiveOfs, receive_size,
                               &num_received) == USYNERGY_FALSE) {
        /* Receive failed, let's try to reconnect */
        char buffer[128];
        sprintf(buffer, "Receive failed (%d bytes asked, %d bytes received), trying to reconnect in a second",
    ...
    if (packlen > USYNERGY_RECEIVE_BUFFER_SIZE) {
        ...
        sprintf(buffer, "Oversized packet: '%c%c%c%c' (length %d)", context->m_receiveBuffer[4],
                context->m_receiveBuffer[5], context->m_receiveBuffer[6], context->m_receiveBuffer[7], packlen);
        sTrace(context, buffer);
        ...
        while (num_received != packlen) {
            ...
            if (context->m_receiveFunc(context->m_cookie, context->m_receiveBuffer, to_receive, &ditch_received) ==
                USYNERGY_FALSE) {
                /* Receive failed, let's try to reconnect */
