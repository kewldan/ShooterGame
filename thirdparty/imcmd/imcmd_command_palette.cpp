...
namespace ImCmd
{
// Compatibility shims for newer Dear ImGui versions (ImFont::FontSize / io.FontGlobalScale were
// obsoleted in 1.92; GetWindowContentRegionMin/Max in 1.90).
#if IMGUI_VERSION_NUM >= 19200
static float ImCmd_FontSize(ImFont*) { return ImGui::GetFontSize(); }
static float ImCmd_FontScale() { return 1.0f; }
#else
static float ImCmd_FontSize(ImFont* font) { return font->FontSize; }
static float ImCmd_FontScale() { return ImGui::GetIO().FontGlobalScale; }
#endif
// =================================================================
...
void CommandPalette(const char* name)
{
    ...
    float font_scale = ImCmd_FontScale();
    ...
    for (int i = 0; i < item_count; ++i) {
        ...
        ImVec2 size{
            ...
            ImMax(ImCmd_FontSize(font_regular), ImCmd_FontSize(font_highlight)),
        };
        ImRect rect{
            window->DC.CursorPos,
        ...
        if (gi.Search.IsActive()) {
            ...
            auto DrawCurrentRange = [&]() {
                if (range_begin != last_range_end) {
                    ...
                    auto segment_size = font_regular->CalcTextSizeA(ImCmd_FontSize(font_regular), std::numeric_limits<float>::max(), 0.0f, begin, end);
                ...
                draw_list->AddText(font_highlight, ImCmd_FontSize(font_highlight) * font_scale, text_pos, text_color_highlight, begin, end);
                auto segment_size = font_highlight->CalcTextSizeA(ImCmd_FontSize(font_highlight) * font_scale, std::numeric_limits<float>::max(), 0.0f, begin, end);
